#!/bin/bash
# zabbix memcached monitor with discovery multi port
# cz20160826
...
cp -a /usr/local/zabbix_memcached/templates/userparameter_discovery_memcached.conf $ZABBIX_DIR/
echo "1) # cp -a /usr/local/zabbix_memcached/templates/userparameter_discovery_memcached.conf $ZABBIX_DIR/"
echo
chmod +x /usr/local/zabbix_memcached/bin/*.sh
echo "2) # chmod +x /usr/local/zabbix_memcached/bin/*.{sh}"
echo
chmod +s /bin/netstat
